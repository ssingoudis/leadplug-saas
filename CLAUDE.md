...
- `aufgabe_52d_drop_skip_submit_step` (**vorbereitet, NOCH NICHT angewendet**): `ALTER TABLE funnels DROP COLUMN skip_submit_step`. Alle Code-Referenzen sind in 52D bereits entfernt. **Erst nach Merge+Deploy des 52D-Codes anwenden** (sonst 500 im alten Prod-Code, der die Spalte noch im SELECT hat). UP+DOWN liegen im Repo. Nach Anwendung: `supabase-schema.md` neu generieren.

**Nächste DB-Arbeit:** den gestageten `aufgabe_52d_drop_skip_submit_step`-DROP nach dem Deploy anwenden. `footer_*`-Spalten + orphaned Submit-Pages sind bereits weg (52B/52D).
