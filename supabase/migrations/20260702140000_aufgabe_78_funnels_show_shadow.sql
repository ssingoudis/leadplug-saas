-- =============================================================================
-- Aufgabe 78 — Einbett-Feinschliff: Schatten abschaltbar (UP)
--
--   • show_shadow — Karten-Schatten des Widgets an/aus (Design-Panel).
--     Baustein für nahtloses Einbetten: Schatten aus + Funnel-Hintergrund in
--     der Farbe der Eltern-Seite ⇒ Funnel wirkt wie Teil der Seite.
--
-- NOT NULL DEFAULT true (Muster der Aufgabe-56-Design-Schalter) — bestehende
-- Funnels behalten exakt das heutige Verhalten; alter deployter Code selektiert
-- Spalten explizit und ist unberührt.
--
-- Zusätzlich: duplicate_funnel kopiert show_shadow mit (explizite Spalten-Liste,
-- Lehre aus dem Aufgabe-77-Review). snapshot/create_from_template tragen die
-- Design-Spalten-Nachzügler weiterhin gesammelt nach, wenn Vorlagen sie brauchen
-- (siehe context/current-feature.md „Offen") — Instanzen erhalten bis dahin den
-- Default true, was dem Standard-Look entspricht.
--
-- Rollback: 20260702140000_aufgabe_78_funnels_show_shadow_DOWN.sql
-- =============================================================================

ALTER TABLE funnels
  ADD COLUMN show_shadow boolean NOT NULL DEFAULT true;
...
begin
  ...
  insert into funnels (
    ...
    font, border_radius, max_width, icon_color, show_shadow
  ) values (
    ...
    v_src.icon_color, v_src.show_shadow
