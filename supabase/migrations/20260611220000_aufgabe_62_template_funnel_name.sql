-- =============================================================================
-- Aufgabe 62 Runde 3 — Namens-Abfrage beim „Vorlage verwenden" (UP)
--
-- `create_funnel_from_template` bekommt einen optionalen p_funnel_name:
-- die UI fragt (wie beim leeren Funnel) zuerst nach dem Namen; ohne Angabe
-- fällt der Funnel-Name wie bisher auf den Vorlagen-Namen zurück.
--
-- Signatur-Wechsel: Postgres-Funktionen sind über die Signatur identifiziert —
-- alte 3-Param-Fassung droppen, 4-Param-Fassung anlegen, Grants neu setzen.
--
-- Rollback: 20260611220000_aufgabe_62_template_funnel_name_DOWN.sql
-- =============================================================================
...
create or replace function public.create_funnel_from_template(
  ...
  p_funnel_name        text default null
) returns text
language plpgsql
...
begin
  ...
  insert into funnels (
    ...
    v_slug, p_tenant_id,
    coalesce(nullif(trim(p_funnel_name), ''), v_tpl.name),
    trim(p_notification_email), true,
  ...
  -- page-Ziel werden übersprungen — die Runtime würde sie ohnehin degradieren.
  ...
  -- Drip-Mails (Teil des Template-Werts).
...
revoke execute on function public.create_funnel_from_template(text, uuid, text, text) from public, anon;
grant execute on function public.create_funnel_from_template(text, uuid, text, text) to authenticated;
