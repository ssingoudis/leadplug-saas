...
export async function POST(req: Request) {
  ...
  // 3. PDF generieren
  const pdfBuffer = await generatePDF({ contact, answers, estimate, tenantConfig })
  ...
  await sendAllEmails({
    contact, answers, estimate, tenantConfig, pdfBuffer, monthlyCount
  })
...
export function calculateEstimate(
  ...
  const storageAddon = answers.stromspeicher === 'ja' ? pricing.storageAddon : 0
  const total = base + storageAddon
  return {
    min: Math.round(total * 0.90),   // -10%
...
## 15. Spätere Erweiterungen (nicht jetzt umsetzen)
