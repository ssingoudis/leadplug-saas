...
| **Aufgabe 41** ✅ | **E-Mails als Drip-Action-Element (2026-05-31)** — komplettes Drip-System für Lead-Nurturing: zeitversetzte Mail-Sequenzen (delay_minutes), TipTap-WYSIWYG-Editor mit Variable + Magic-Section Chips, 3-Pane In-Place-Editor (Liste · Editor · Live-Vorschau), Auto-Save 1.5s Debounce, Vorschau mit Mock- oder echten Lead-Daten. Schema `email_subscriptions` + `email_delivery_attempts` mit Queue-Pattern (scheduled_at, status pending/retrying/success/failed). Backfill: 2 Default-Subscriptions pro Funnel (24 Rows angelegt für 12 Funnels). Cron erweitert um due-pending + due-retrying. Hartkodierter `sendAllEmails`-Pfad in `/api/submit` entfernt. Branch `feature/aufgabe-41-emails-tab`. **Strategischer Pivot mittendrin:** ursprünglich als 1:1-Klon des Webhook-Tabs gebaut, dann zum Drip-Modell umgebaut (E-Mails ≠ Webhooks). | ~2.5 Tage |
...
| **D.2** ✅ | **Conversion-Tracking via postMessage + Script-Loader-Embed (Aufgabe 42, 2026-05-31)** — Modell A „event-basiert": Widget meldet Submit PII-frei per `postMessage`, zentraler `embed.js`-Loader feuert Conversions (GTM-`dataLayer`-Push `leadplug_lead` + optionale `data-meta-lead`/`data-google-conversion`-Attribute + `window.LeadPlug.onLead`). `<div data-leadplug="slug"></div><script src=".../embed.js" defer>` ersetzt das iFrame-Snippet als empfohlenen Weg (zentral updatebar); iFrame bleibt Fallback. Keine Pixel-IDs in DB/Editor — technische Agenturen verdrahten in GTM. Server-CAPI bewusst on-demand. Branch `feature/aufgabe-42-conversion-tracking`. Details: [`conversion-tracking.md`](conversion-tracking.md). **Performance-Marketing-Blocker erledigt.** **+ Aufgabe 43 (Turnkey):** Pixel-ID-Felder pro Funnel im Editor-Reiter „Einbinden" (`funnels.meta_pixel_id`/`google_ads_conversion`), `embed.js` feuert automatisch inkl. Basiscode-Injection. Globale `/dashboard/embed`-Seite entfernt (Tracking ist pro Funnel). | ~1,5 Tage |
