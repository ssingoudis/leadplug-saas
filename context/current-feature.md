...
- **UI-Restyling + Icon-Migration** – Icons auf Lucide React umgestellt (`icons.tsx` komplett neu, `ICON_MAP` exportiert, `renderIcon` nimmt jetzt `color` statt `iconProps`). `iconProps` aus `Option`-Type entfernt. Option-Cards: `borderRadius` aus DB-Theme, Shadow, Hover zeigt vollen Selected-Look (Border + Tint + Shadow), Radio-Kreis entfernt. Icon-Wrapper mit `primaryColor`-Tint-Hintergrund. Progress-Bar dicker (`h-2.5`), %-Anzeige entfernt. Widget-Container: Shadow `0 8px 24px`, `borderRadius` und `overflow:hidden` aus Theme. `pageBackgroundColor` Default auf `transparent` korrigiert, `backgroundColor` Default auf `#ffffff`. `pageBackgroundColor`-Wrapper auch auf Success-Screen angewendet. `app/icons/page.tsx` auf `ICON_MAP` umgestellt. **DB-Aktion ausstehend:** `icon_key`-Werte in `funnel_questions.options` von alten Namen (`House`→`Home`, `Apartment`→`Building2` etc.) auf Lucide-Namen updaten. (`components/funnel.tsx`, `components/icons.tsx`, `types/index.ts`, `app/icons/page.tsx`)
- **Aufgabe 8 – Deprecated Files entfernt, Build geprüft** – `lib/generatePDF.ts` und `lib/priceCalculator.ts` gelöscht (keine Importe mehr vorhanden). `tsc --noEmit` → 0 Fehler, `npm run build` → erfolgreich. (`lib/generatePDF.ts`, `lib/priceCalculator.ts`)
