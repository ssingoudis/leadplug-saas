...
- Tracking-Erweiterung MVP (project-overview.md #22) – submissions-Tabelle um 4 Spalten erweitert (source_url/user_agent/started_at/price_per_lead NUMERIC(10,4) DEFAULT 0.10); ALTER TABLE ADD COLUMN IF NOT EXISTS für idempotente Re-Runs; monthly_billing-View nutzt jetzt SUM(price_per_lead) statt COUNT(*)*0.10; neuer BillingConfig-Typ + optionales billing-Feld auf TenantConfig; SubmitPayload um startedAt/sourceUrl/userAgent erweitert; TenantFunnelClient setzt startedAt via useRef-Lazy-Init beim Mount und liest document.referrer + navigator.userAgent beim Submit; API-Route liest pricePerLead server-side aus tenantConfig.billing?.pricePerLead ?? 0.10 (manipulationssicher) und reicht alle neuen Felder an logSubmission durch; tolerant bei fehlenden Client-Feldern (Fallback auf Leerstrings bzw. new Date().toISOString()); demo tenant 0,10 €/Lead, musterfirma 0,20 €/Lead als Beispiel flexibler Bepreisung; tsc + next build grün; Supabase-SQL-Run + env-Vars steht als manuelle Agenda in #22 (context/project-overview.md, context/supabase-schema.sql, types/index.ts, tenants/_template.json, tenants/demo.json, tenants/musterfirma.json, components/TenantFunnelClient.tsx, lib/tracking.ts, app/api/submit/route.ts)
